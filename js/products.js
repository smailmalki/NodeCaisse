...
$('.myTableC').on('click', '.clickable-row', function(event) {
		...
		db.each("SELECT * FROM prd where idc="+$(this).find('td:first').html(), function(err, row) {
			$('#TbProduct').append('<tr class="clickable-row d-flex"><td class="col-2">'+row.idp+'</td><td class="col-5">'+row.nom+'</td><td class="col-2">'+row.prix+'</td><td class="col-3 pl-4"><a href="#" data-toggle="modal" data-target="#ModalProduct" onclick="fillmenu('+row.idp+');prdtitle.innerText=\'Modifier produit\';cat.value='+row.idc+';nomP.value=\''+row.nom+'\';price.value='+row.prix+';prdvalid.onclick=function() { updateproduct('+row.idp+') }"><img src="img/pen.svg" width=20></img></a><a href="#" data-toggle="modal" data-target="#popmsg" onclick="poptitle.innerText=\'Supprimer produit\';poptext.innerText=\'Voulez-vous supprimer le produit : '+row.nom+' ?\';popvalid.onclick=function() { supprprd('+row.idc+','+row.idp+') }"><img src="img/trash.png" width=20></img></a></td></tr>');		
		});
	  });
...
function updateproduct(idp) {
	if (nomP.value=="") {
		...
		sqlite.open('db/caisse.db').then(db2=>{
			...
			.then(() => {
				db2.run("update prd set idc="+cat.value+",nom='"+nomP.value+"',prix="+price.value.replace(",",".")+", menu="+menu+" where idp="+idp+" ;").then( () => {
			...
			if (row2 !== undefined) {
				...
				db.get("select max(idp+10) mx from prd", (err, row) => {
					...
					sqlite.open('db/caisse.db').then(db2=>{
						...
						.then(() => {
							db2.run("INSERT INTO prd VALUES ("+mx+", "+cat.value+",'"+nomP.value+"',"+price.value.replace(",",".")+",'',"+menu+");").then( () => {
								document.getElementById('cat'+cat.value).click();
								$("#ModalProduct").modal("hide");
							});
						})
